...
if __name__ == "__main__":
    ...
    for i in range(n_epochs+1):
        ...
        for j, (x, label) in enumerate(train_loader):
            ...
            loss_discriminator2 = Variable(loss_discriminator, requires_grad=True)
            loss_decoder = torch.sum(lambda_mse * mse) - (1.0 - lambda_mse) * loss_discriminator2
            loss_decoder2 = Variable(loss_decoder, requires_grad=True)
            ...
            #decoder
            if train_dec:
                loss_decoder2.backward(retain_graph=True)  #[p.grad.data.clamp_(-1,1) for p in net.decoder.parameters()]
                optimizer_decoder.step()
                net.discriminator.zero_grad()  #clean the discriminator
                
            #loss_discriminator = Variable(torch.sum(bce_dis_original) + torch.sum(bce_dis_predicted) + torch.sum(bce_dis_sampled), requires_grad=True)

            #discriminator
            if train_dis:
                loss_discriminator2.backward()  #[p.grad.data.clamp_(-1,1) for p in net.discriminator.parameters()]
                optimizer_discriminator.step()

            print('Epoch[%d/%d] Image[%d/%d] encoder loss: %.5f | decoder loss: %.5f | discriminator loss: %.5f' % (i, n_epochs+1, j, len(train_loader), loss_encoder, loss_decoder, loss_discriminator))
